...
def parse_cyclization_info(cyclization_str: str) -> List[Tuple[int, int]]:
    ...
    for item in cyclization_str.split(","):
        ...
        if "-" not in item:
            raise ValueError(
                f"Invalid bridge format: '{item}'. Expected format like '2-4'."
            )
        a, b = item.split("-", 1)
...
def build_candidate_templates(
    template_seq: str, bridges: List[Tuple[int, int]]
) -> Tuple[str, List[str]]:
    n = len(template_seq)
...
def needleman_wunsch(
    ...
    aligned1 = list(reversed(aligned1))
    aligned2 = list(reversed(aligned2))
    aligned_query = "".join(aligned1)
    aligned_template = "".join(aligned2)

    match_line: List[str] = []
    matches = 0
    non_gap_compared = 0
    total_alignment_len = len(aligned_query)

    for a, b in zip(aligned_query, aligned_template):
        if a == b and a != "-":
            match_line.append("|")
            matches += 1
            non_gap_compared += 1
